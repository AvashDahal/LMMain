...
router.post("/admin/books/add", middleware.ensureAdminLoggedIn, async (req,res) => {
	try
	{
		...
		{
			req.flash("error", "ISBN should be of length 5");
...
router.put("/admin/book/:bookId", middleware.ensureAdminLoggedIn, async (req,res) => {
	...
	if(updateObj.ISBN.toString().length != 5)
	{
		req.flash("error", "ISBN should be of length 5");
		return res.redirect("back");
	}
...
router.get("/admin/emails/reminder", middleware.ensureAdminLoggedIn, async (req,res) => {
	try
	{
		...
		const transport = nodemailer.createTransport({
			service: "gmail",
			auth: {
				user: "bijaydahal070@gmail.com",
				pass: "odos ybzr jibz gueo"
			}
		});
		...
		for(let i=0; i<currentLoans.length; i++)
		{
			...
			await transport.sendMail({
				...
				html: `<p>Your book ${book} is due to be returned before ${dueTime}.
				Please visit library and return it as soon as possible otherwise fine will be charged as per the rules!!!</p>`
			});
